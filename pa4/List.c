...
void deleteFront(List L){
	if(length(L)>0){
		if(L->cursor == L->front){
			L->cursor = NULL;			//case of cursor node being deleted
			L->index = NULL;
		}
		
		Node temp = L->front;			//front node
		L->front = temp->next;		//setting new front
		if(L->front != NULL){
			L->front->prev = NULL;
		}
